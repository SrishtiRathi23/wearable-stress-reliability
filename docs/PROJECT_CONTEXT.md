...
**Study A - Measurement / evaluation distortion (frozen design: `docs/study_a_protocol.md`, D-033/D-034).** Fix the frozen Phase-3 out-of-participant predictions (SHA `34010de9...2212`). Vary only the label-observation mask: a common targeted policy (highest consensus stress rank of the frozen LR/RF/XGB probabilities within each participant's complete recording; self-consistent model-triggered batch acquisition, offline) versus uniform same-budget random acquisition (500 dataset-level replicates), at budgets 25/50/75 % of each participant's complete 60-s windows (50 % primary; 100 % negative control), one annotation unit per selected window including out-of-target windows. Endpoints: conditional participant-level BA distortion contrast C_BA (H-A1, primary at 50 %), estimability contrast C_E (H-A2), dataset-level model-selection change and empirical full-cohort selection regret (H-A3), Brier distortion C_BS (H-A4; log loss secondary). The candidate unit is a single complete 60-s time-grid window; reference labels never enter candidate generation or the selector. *Historical planning note (superseded by D-033/D-034): earlier text listed detector-triggered, duration-preferred and mixed policies at event/episode/block level; those are not part of confirmatory Study A - `mixed` belongs to Study C (T-09).*
