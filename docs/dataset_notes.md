...
- Built by `python -m wsr.features.build_features` from the verified raw tree (loader re-verifies the baseline and every pickle hash; D-022). Manifest: `data/manifests/wesad_windows_60s_manifest.json` (table SHA-256 `ebc0ccbde2062a77...` under schema 1.1.0; the 1.0.0 table `f786c304c5884a0d...` differed only by the name of one quality column, counts, parameters, package versions). The parquet itself is regenerable and not committed.
...
- **Structural quality:** every window has the expected native sample counts (1920/3840/240/240), all finite, no constant signals, zero feature-extraction errors; EDA decomposition and pulse detection ran for all 15. `q_acc_near_rail` 12.3 % (KI-22; schema 1.1.0 renamed it from `q_acc_clipped`); `q_eda_below_plausible` 0 %; `q_temp_out_of_range` 0 %.
